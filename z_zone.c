...
boolean Z_InitXms(uint32_t size)
{
	...
	XMSControl = kMK_FP(sregs.es, regs.w.bx);
...
void Z_Init (void)
{
	...
	// normalize pointer
	mainzone_sentinal = kMK_FP(kFP_SEG(mainzone_sentinal) + kFP_OFF(mainzone_sentinal) / PARAGRAPH_SIZE, 0);
#endif
...
#if defined _M_I86
	block = (memblock_t __far*)(((uint32_t)ptr) - 0x00010000);
#else
	block = (memblock_t __far*)(((uint32_t)ptr) - 0x00010);
#endif
...
static void __far* Z_TryMalloc(uint16_t size, int8_t tag, void __far*__far* user)
{
    ...
    if (user)
        ...
        base->user = (void __far*__far*) kMK_FP(0,2); // unowned
#if defined ZONEIDCHECK
    base->id  = ZONEID;
#endif
...
{
	if (size >= 0x10000)
		I_Error("Z_MallocStatic: %li is too big", size);
...
{
	if (size >= 0x10000)
		I_Error("Z_MallocStaticWithUser: %li is too big", size);
